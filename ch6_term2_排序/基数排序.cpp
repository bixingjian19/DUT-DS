...
// 操作结果: 进行第i趟分配
{
    for (int index = 0; index < n; index++) //遍历data中的所有元素
    {
        ElemType cur = elem[index];
        int power = i - 1;
        int divide = pow(r, power);
        cur = cur / divide;
        cur = cur % r;
        list[cur].Insert(list[cur].Length() + 1, elem[index]);
    }
}
...
// 操作结果: 进行第i趟收集
{
    ...
    for (int j = 0; j < r; j++)
    {
        ...
        while (k < len)
        {
            list[j].GetElem(k + 1, elem[index]); //复制对应数据
            index++;
            k++;
        }
    }
...
*/
template <class ElemType>
void RadixSort(ElemType elem[], int n, int r, int d) //n = 6, r = 10, d = 2;
// 初始条件: r为基数,d为关键字位数
// 操作结果: 对elem进行基数排序
{
    ...
    for (int i = 0; i < d; i++)
    {
        ...
        Distribute<ElemType>(elem, n, r, d, i + 1, list);
        Colect<ElemType>(elem, n, r, d, i, list);
    }
}
